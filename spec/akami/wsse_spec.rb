  ...
  describe "#to_xml" do
    ...
    context "whith signature and timestamp on it" do
      ...
      before do
        ...
        allow(wsse.signature).to receive(:have_document?).and_return(true)
        allow(wsse.signature).to receive(:body_digest).and_return("stubbedBodyDigest")
        allow(wsse.signature).to receive(:timestamp_digest).and_return("stubbedTimestampDigest")
      end
      ...
      it "contains an id on Timestamp" do
        ...
        expect(wsse.to_xml).to include(%(<wsu:Timestamp wsu:Id="#{id}"))
      end

      it "contains two references" do
        body_id = wsse.signature.body_id
        expect(wsse.to_xml).to include(%(<Reference URI="##{body_id}"))
        timestamp_id = wsse.signature.timestamp_id
        expect(wsse.to_xml).to include(%(<Reference URI="##{timestamp_id}"))
      end
